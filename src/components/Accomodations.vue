...
export default {
  ...
  mounted () {
    axios.get('http://localhost:8080/accomodations/').then(response => {
  ...
  methods: {
    ...
    deleteItem (item) {
      ...
      if (confirm('Are you sure you want to delete this ride?')) {
        axios
          .delete('http://localhost:8080/accomodations/' + item._id)
    ...
    saveItem () {
      ...
      if (this.editedIndex > -1) {
        if (this.$refs.form.validate()) {
          // Edit existing item
          axios.put('http://localhost:8080/accomodations/' + this.editedItem._id, {
